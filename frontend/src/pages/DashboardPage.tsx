...
export default function DashboardPage() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 h-full min-h-0 overflow-hidden animate-in fade-in duration-700">
      ...
      <div className="lg:col-span-3 flex flex-col gap-6 h-full min-h-0 overflow-hidden">
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 min-h-0 overflow-hidden">
          ...
          {/* MASTERY FEED (LIGHT GRAY BG, DARK TEXT) */}
          <Card className="bg-slate-100 border-slate-200 flex flex-col h-full shadow-inner min-h-0 overflow-hidden">
            <CardHeader className="shrink-0 border-b border-slate-200 pb-4 bg-slate-200/30">
              <div className="flex justify-between items-center">
                <CardTitle className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                  <History className="size-4 text-slate-500" /> Mastery
                </CardTitle>
      ...
      {/* 3. WIDE COURSE LIBRARY (WHITE BG, BLACK TEXT) */}
      <Card className="lg:col-span-2 h-full bg-white border-slate-200 flex flex-col shadow-xl min-h-0 overflow-hidden">
        ...
        {/* Adjusted padding: px-4 pb-4 pt-2 to reduce space at the top */}
        <CardContent className="flex-1 overflow-y-auto px-4 pb-4 pt-2 space-y-3 scrollbar-thin scrollbar-thumb-slate-200 min-h-0">
          {courses.map((c) => (
...
function MasteryItem({
  ...
  return (
    <button className="w-full text-left p-3.5 rounded-xl bg-white hover:bg-slate-50 transition-all border border-slate-200 hover:border-slate-300 group flex flex-col gap-2.5 shadow-sm shrink-0 h-[68px]">
      <div className="flex justify-between items-center w-full">
        <span className="text-sm font-bold text-slate-900 transition-colors flex items-center gap-2 leading-none">
          {topic}
          <ArrowRight className="size-3.5 text-slate-400 opacity-0 group-hover:opacity-100 group-hover:text-slate-900 transition-all -translate-x-2 group-hover:translate-x-0" />
        </span>
        <span
          className={cn(
      ...
      <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden border border-slate-200 shrink-0">
        <div
          className={cn(
